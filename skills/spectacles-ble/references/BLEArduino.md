...
export class BLEArduino extends BaseScriptComponent {
  ...
  private startScan() {
    ...
    this.bluetoothModule
      .startScan([this.scanFilter], this.scanSetting, (scanResult: Bluetooth.ScanResult) => {
  ...
  private async connectGATT(scanResult: Bluetooth.ScanResult) {
    ...
    const desiredService = gatt.getService(SERVICE_UUID)
    const desiredChar = desiredService.getCharacteristic(CHARACTERISTIC_UUID)
      ...
      if (connectionState.state == Bluetooth.ConnectionState.Connected) {
        ...
        desiredChar
          .registerNotifications((value) => {
            const message = this.bin2str(value)
